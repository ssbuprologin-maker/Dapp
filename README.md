...
The header bell is separate from transient alerts. Reply notices are cached locally, while welcome and moderation notices are stored in Redis. `/api/moderation?network=<network>&wallet=<wallet>` must accept both JSON strings and already-decoded objects returned by Upstash. The client polls every 5 seconds, deduplicates by notification ID, and shows new warnings/timeouts in both the bell list and the alert stack. If moderation notifications stop appearing, verify the two Upstash REST environment variables, inspect `testnet-games:notifications:<network>:<wallet>`, and confirm the latest Vercel deployment includes `api/moderation.ts`.
