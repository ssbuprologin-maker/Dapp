...
- Other players' profiles open as a centered, internally scrollable modal over a dimmed game screen; opening one must not unmount the game underneath. The backdrop and Escape key close that modal. The connected player's own profile uses the original full-page layout inside the main content area instead of the popout.
...
Clicking anywhere on an authenticated chat message card except its avatar or username opens Check profile, Mute locally, and Reply actions in a fixed foreground menu at the click coordinates. The menu opens below the pointer when it fits and directly above it near the bottom edge. The expanded chat container must not retain a CSS transform, because that would offset and clip the fixed menu. Clicking the name or avatar goes directly to the wallet's profile. Replies carry a short sanitized quote of the selected message. Local mute state is scoped to the current viewer wallet and device and can be reversed in Profile Settings. Tipping is launched from the target player's profile. Tips are non-custodial, same-network native transfers: devnet SOL to Solana players or MegaETH testnet ETH to MegaETH players. The sender confirms the exact recipient and amount in Phantom, Solflare, MetaMask, or the browser-local site wallet. Tips are irreversible, have no house fee, and are not stored by the server. The recipient address originates from the authenticated chat/profile identity, not arbitrary tip-form input.

### Alerts and notifications

`App.tsx` owns the global right-side alert stack. Use `showSideAlert(title, message, tone)` for transient errors or warnings raised by child features. Chat passes failures upward through its `onError` callback, including cooldown, Ably connection, authorization, and publish failures. Moderation catches also report globally before rethrowing so the on-site modal can retain its inline explanation.

Alerts use `error`, `warning`, or `success` tones, remain visible for about 6.5 seconds, then enter an `is-closing` state for 320 ms before removal. The close button uses the same exit state. Do not remove an alert from state immediately or the exit animation cannot run. The CSS animations are `side-alert-in` and `side-alert-out`, with a reduced-motion override.

The header bell is separate from transient alerts. Clicking an inbox row marks it read and opens its complete message inside the compact dropdown; it no longer redirects to a profile. The header action is **Clear**, which empties the inbox and records a per-wallet local timestamp under `testnet-games:notifications-cleared:v1:<network>:<wallet>` so Redis polling does not restore older messages. Reply notices are cached locally, while welcome and moderation notices are stored in Redis. `/api/moderation?network=<network>&wallet=<wallet>` must accept both JSON strings and already-decoded objects returned by Upstash. The client polls every 5 seconds, deduplicates by notification ID, preserves local read state, and shows new warnings/timeouts in both the bell list and the alert stack. If moderation notifications stop appearing, verify the two Upstash REST environment variables, inspect `testnet-games:notifications:<network>:<wallet>`, and confirm the latest Vercel deployment includes `api/moderation.ts`.

Profile Settings includes a large clickable avatar tile with an edit-image badge, nickname and referral-code save fields, plus a disabled Account Email field marked Coming soon. Referral codes are currently saved locally on the player’s device, matching the existing affiliate page. Profile pictures can be dragged horizontally and vertically inside the crop square with mouse or touch, then adjusted with the zoom slider before saving. The level and XP progress card stays visible above every profile tab. Clicking the DINOGAME logo takes a connected player directly to the game screen. Chat messages use larger avatars, names, text, and message cards on desktop.
