...
type ScoreRow = { rank: number; score: number; playedAt: number; won: boolean; transaction?: string; network?: PaymentNetwork; wallet?: string; profileWallet?: string; level?: number }
...
export default function SingleplayerDinoGame({ address, paymentNetwork, localWallet, sendTransaction, signTransaction, connection, onBalanceSpent, onViewProfile, onExit }: {
  ...
  return <section className="game-page">
    <div className="game-header"><div><span>DUAL TESTNET BOT RACE - BUILD V51</span><h1>Dino Run</h1></div><button onClick={onExit}>View profile</button></div>
    <div className="game-layout">
      ...
      <aside className="players-card"><div className="players-title"><div><span>{leaderboardMode === 'worldwide' ? 'UPSTASH REDIS' : 'THIS SESSION'}</span><h2>{leaderboardMode === 'worldwide' ? 'Worldwide leaderboard' : 'Session high scores'}</h2></div><i className={leaderboardMode === 'worldwide' ? 'online' : ''} /></div><div className="leaderboard-tabs"><button className={leaderboardMode === 'worldwide' ? 'active' : ''} onClick={() => setLeaderboardMode('worldwide')}>Worldwide</button><button className={leaderboardMode === 'local' ? 'active' : ''} onClick={() => setLeaderboardMode('local')}>Session</button></div><div className="leaderboard score-board">{leaderboard.length ? <><div className="score-header"><b>#</b><strong>PLAYER</strong><em>SCORE</em><span>WIN</span><i>TX</i></div>{leaderboard.map(row => <div key={`${row.transaction ?? row.playedAt}-${row.rank}`}><b>#{row.rank}</b><div className="leaderboard-player">{row.profileWallet && row.network ? <button className="leaderboard-name" onClick={() => onViewProfile(row.profileWallet!, row.network!)}>{row.wallet ?? 'Player'}</button> : <strong>{row.wallet ?? 'You'}</strong>}{typeof row.level === 'number' && <mark className={`level-badge level-tier-${levelTier(row.level)}`}>LV {row.level}</mark>}</div><em>{Math.floor(row.score / 1000)}m</em><span className={row.won ? 'win-yes' : 'win-no'}>{row.won ? 'Yes' : 'No'}</span>{row.transaction && row.network ? <a href={row.network === 'solana' ? `https://explorer.solana.com/tx/${row.transaction}?cluster=devnet` : `${MEGAETH_EXPLORER_URL}/tx/${row.transaction}`} target="_blank" rel="noreferrer" title={`Open ${row.network === 'solana' ? 'Solana Explorer' : 'MegaETH Blockscout'}`}>View</a> : <i>—</i>}</div>)}</> : <p>{leaderboardMode === 'worldwide' ? 'No worldwide scores yet.' : 'Finish a run to record the first session score.'}</p>}</div><div className="server-note"><ShieldCheck /><p><strong>{leaderboardMode === 'worldwide' ? 'Worldwide scores' : 'Session scores'}</strong><span>{leaderboardMode === 'worldwide' ? 'Paid entries are verified before Redis accepts a score.' : 'These scores are saved only for this browser session.'}</span></p></div></aside>
    </div>
  </section>
}
