...
export default function ProfilePage({ initialSection, wallet, network, displayName, canChangeName, savingName, nextNameChangeAt, onChangeName, onBack }: { initialSection: ProfileSection; wallet: string; network: Network; displayName: string; canChangeName: boolean; savingName: boolean; nextNameChangeAt: number; onChangeName: (name: string) => Promise<void>; onBack: () => void }) {
  ...
  return <><section className="profile-page">
    ...
    <div className="profile-hero"><button className="profile-avatar" onClick={() => avatarInput.current?.click()} title="Upload profile picture">{avatar ? <img src={avatar} alt="Profile" /> : <UserRound />}<span>EDIT</span></button><input ref={avatarInput} className="avatar-file-input" type="file" accept="image/png,image/jpeg,image/gif,image/webp" onChange={event => { uploadAvatar(event.target.files?.[0]); event.target.value = '' }} /><div><small>{network.toUpperCase()} WALLET</small><code>{wallet}</code><p>Click your profile picture to upload and crop an image.</p></div></div>
    <nav className="profile-nav"><button className={section === 'statistics' ? 'active' : ''} onClick={() => setSection('statistics')}>Statistics</button><button className={section === 'transactions' ? 'active' : ''} onClick={() => setSection('transactions')}>Transactions</button><button className={section === 'settings' ? 'active' : ''} onClick={() => setSection('settings')}>Settings</button></nav>
