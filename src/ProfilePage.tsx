...
import { ArrowLeft, BadgeCheck, Copy, ExternalLink, Gamepad2, Image, MessageCircle, ShieldAlert, Trophy, UserRound, Wallet, X } from 'lucide-react'
import { MEGAETH_EXPLORER_URL } from './megaEth'
import { chatLevelTier, levelTier } from './leveling'
import type { TipTarget } from './TipModal'

type Network = 'solana' | 'megaeth'
type ProfileSection = 'statistics' | 'transactions' | 'settings'
type ProfileData = { displayName: string; avatarUrl: string; referralCode: string; createdAt: number; verified: boolean; moderator: boolean; streamer: boolean; kickSlug: string; kickUrl: string; warningCount: number; discordConnected: boolean; nextChangeAt: number; level: number; wagerEquivalentSol: number; wagerIntoLevelSol: number; wagerForNextLevelSol: number; stats: { gamesPlayed: number; wins: number; losses: number; bestScore: number; solWagered: number; ethWagered: number }; transactions: { hash: string; network: Network; playedAt: number; score: number; won: boolean }[] }

export default function ProfilePage({ isOwn, initialSection, wallet, network, displayName, canChangeName, savingName, nextNameChangeAt, onChangeName, onSetReferralCode, onChangeAvatar, onChangeKickChannel, onTipPlayer, canModerate, onModerate, onBack }: { isOwn: boolean; initialSection: ProfileSection; wallet: string; network: Network; displayName: string; canChangeName: boolean; savingName: boolean; nextNameChangeAt: number; onChangeName: (name: string) => Promise<void>; onSetReferralCode: (code: string) => Promise<string>; onChangeAvatar: (avatar: string) => Promise<void>; onChangeKickChannel: (url: string) => Promise<string>; onTipPlayer: (target: TipTarget) => void; canModerate: boolean; onModerate: (target: { wallet: string; network: Network; name: string; action: 'warn' | 'timeout' }, note: string, durationMinutes: number) => Promise<void>; onBack: () => void }) {
  const [profile, setProfile] = useState<ProfileData | null>(null)
  const [name, setName] = useState(displayName)
  const [avatar, setAvatar] = useState(() => localStorage.getItem(`testnet-games:avatar:${network}:${wallet}`) ?? '')
  const [avatarStatus, setAvatarStatus] = useState('')
  const [joinedReferralInput, setJoinedReferralInput] = useState('')
  const [joinedReferralStatus, setJoinedReferralStatus] = useState('')
  const [savingJoinedReferral, setSavingJoinedReferral] = useState(false)
  ...
  const [chartRange, setChartRange] = useState<'24H' | '7D' | '1M' | 'ALL'>('24H')
  ...
  const copyWalletAddress = async () => {
    ...
    } catch { setWalletCopied(false) }
  }
  const saveJoinedReferral = async (event: FormEvent) => {
    event.preventDefault()
    const code = joinedReferralInput.toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 12)
    if (code.length < 3) { setJoinedReferralStatus('Use at least three letters or numbers.'); return }
    setSavingJoinedReferral(true); setJoinedReferralStatus('')
    try {
      const saved = await onSetReferralCode(code)
      setProfile(current => current ? { ...current, referralCode: saved } : current)
      setJoinedReferralInput(saved)
      setJoinedReferralStatus('Referral code saved and permanently locked.')
    } catch (error) { setJoinedReferralStatus(error instanceof Error ? error.message : 'Could not save referral code.') }
    finally { setSavingJoinedReferral(false) }
  }
  ...
  const runModeration = async () => {
    ...
    try { await onModerate({ wallet, network, name: playerName, action: moderationAction }, moderationNote.trim(), moderationAction === 'timeout' ? timeoutMinutes : 0); setProfile(current => current ? { ...current, warningCount: current.warningCount + 1 } : current); setModerationAction(null); setModerationNote('') }
  ...
  const chartPlotLeft = isOwn ? 44 : 16
  const chartPlotRight = isOwn ? 16 : 74
  const chartX = (index: number) => chartPlotLeft + index * (chartWidth - chartPlotLeft - chartPlotRight) / Math.max(1, chartValues.length - 1)
  ...
  const referralSettingsField = profile?.referralCode
    ? <div className="settings-field-form referral-code-locked"><label>JOINED WITH REFERRAL CODE<div><input value={profile.referralCode} disabled readOnly /><button type="button" disabled>Locked</button></div><small>This referral code is permanently attached to your profile.</small></label></div>
    : <form className="settings-field-form" onSubmit={saveJoinedReferral}><label>JOINED WITH REFERRAL CODE<div><input value={joinedReferralInput} onChange={event => setJoinedReferralInput(event.target.value.toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 12))} maxLength={12} placeholder="Set once (optional)" /><button disabled={savingJoinedReferral}>{savingJoinedReferral ? 'Saving…' : 'Save once'}</button></div>{joinedReferralStatus && <small>{joinedReferralStatus}</small>}</label></form>
  return <><section className={`profile-page${isOwn ? '' : ' profile-page-inspected'}`}>
    ...
    <div className="own-profile-summary"><div className="profile-hero"><button className="profile-avatar" onClick={() => avatarInput.current?.click()} title="Change profile picture">{avatar ? <img src={avatar} alt="Profile" /> : <UserRound />}<span className="profile-avatar-edit"><Image /></span></button><input ref={avatarInput} className="avatar-file-input" type="file" accept="image/png,image/jpeg,image/gif,image/webp" onChange={event => { uploadAvatar(event.target.files?.[0]); event.target.value = '' }} /><div><strong className="own-profile-name">{profileName}</strong><small>Joined {profile?.createdAt ? new Date(profile.createdAt).toLocaleDateString() : joinedLabel}</small><code>{wallet}</code>{profile?.kickUrl && <a className="profile-kick-link" href={profile.kickUrl} target="_blank" rel="noreferrer"><img src="/streamer-logo.svg" alt="" /> kick.com/{profile.kickSlug}<ExternalLink /></a>}</div></div><section className={`level-card level-tier-${levelTier(level)}`}><div><span>LEVEL {level}</span><small>{level >= 100 ? 'Maximum level reached.' : `${formatSol(profile?.wagerIntoLevelSol ?? 0)} / ${formatSol(profile?.wagerForNextLevelSol ?? 0.1)} SOL-EQ to the next level`}</small></div><strong>{formatSol(profile?.wagerEquivalentSol ?? 0)} SOL-EQ</strong><div className="level-track"><i style={{ width: `${wagerProgress}%` }} /></div></section></div></> : <header className="inspected-profile-header"><span className="inspected-profile-avatar">{avatar ? <img src={avatar} alt="" /> : <UserRound />}</span><div className="inspected-profile-identity"><div><b className={`chat-level chat-level-${chatLevelTier(level)}`}>{level}</b><h1>{profileName}{profile?.verified && <BadgeCheck className="verified-badge" aria-label="Verified player" />}{profile?.moderator && <b className="moderator-badge" title="Moderator">MOD</b>}{profile?.streamer && <b className="streamer-badge" title="Streamer"><img src="/streamer-logo.svg" alt="" /> STREAMER</b>}</h1></div><small>Joined on {profile?.createdAt ? new Date(profile.createdAt).toLocaleDateString() : joinedLabel}</small>{canModerate && <small className="inspected-profile-warning-count">{profile?.warningCount ?? 0} {(profile?.warningCount ?? 0) === 1 ? 'warning' : 'warnings'}</small>}{profile?.kickUrl && <a className="profile-kick-link" href={profile.kickUrl} target="_blank" rel="noreferrer"><img src="/streamer-logo.svg" alt="" /> kick.com/{profile.kickSlug}<ExternalLink /></a>}</div><div className="inspected-profile-actions"><button type="button" onClick={() => onTipPlayer({ wallet, network, name: playerName, avatar, level, verified: Boolean(profile?.verified) })}>Tip User</button>{canModerate && !profile?.moderator && <><button type="button" className="warn" onClick={() => { setModerationAction('warn'); setModerationError('') }}>Warn</button><button type="button" className="timeout" onClick={() => { setModerationAction('timeout'); setModerationError('') }}>Timeout</button></>}<button type="button" className="inspected-profile-close" onClick={onBack} aria-label="Close player profile"><X /></button></div></header>}
    <div className={`profile-wallet-address${isOwn ? ' own' : ' inspected'}`}><span><small>{network === 'solana' ? 'SOLANA WALLET' : 'MEGAETH WALLET'}</small><code>{wallet}</code></span><button type="button" onClick={() => void copyWalletAddress()} aria-label="Copy wallet address"><Copy />{walletCopied ? 'Copied' : 'Copy'}</button></div>
    ...
    <section className={`profile-performance-chart ${section !== 'statistics' ? 'profile-hidden' : ''}`}>
      {!isOwn && <nav className="profile-chart-ranges" aria-label="Chart range">{(['24H', '7D', '1M', 'ALL'] as const).map(range => <button type="button" key={range} className={chartRange === range ? 'active' : ''} onClick={() => setChartRange(range)}>{range}</button>)}</nav>}
      ...
      <div className="profile-chart-canvas">
        <svg viewBox={`0 0 ${chartWidth} ${chartHeight}`} preserveAspectRatio="none" role="img" aria-label={`Cumulative net result over ${chartGames.length} games`}>
          ...
          {chartGridValues.map(value => <g key={value}><line x1={chartPlotLeft} x2={chartWidth - chartPlotRight} y1={chartY(value)} y2={chartY(value)} className="profile-chart-grid" /><text x={isOwn ? 4 : chartWidth - 62} y={chartY(value) + 4}>{value.toFixed(4)}</text></g>)}
          <line x1={chartPlotLeft} x2={chartWidth - chartPlotRight} y1={chartY(0)} y2={chartY(0)} className="profile-chart-zero" />
