...
export default function ProfilePage({ isOwn, initialSection, wallet, network, displayName, canChangeName, savingName, nextNameChangeAt, onChangeName, onBack }: { isOwn: boolean; initialSection: ProfileSection; wallet: string; network: Network; displayName: string; canChangeName: boolean; savingName: boolean; nextNameChangeAt: number; onChangeName: (name: string) => Promise<void>; onBack: () => void }) {
  ...
  return <><section className="profile-page">
    ...
    <div className="profile-hero"><button className="profile-avatar" onClick={() => isOwn && avatarInput.current?.click()} title={isOwn ? 'Upload profile picture' : 'Player profile picture'}>{avatar ? <img src={avatar} alt="Profile" /> : <UserRound />}{isOwn && <span>EDIT</span>}</button><input ref={avatarInput} className="avatar-file-input" type="file" accept="image/png,image/jpeg,image/gif,image/webp" onChange={event => { uploadAvatar(event.target.files?.[0]); event.target.value = '' }} /><div><small>{network.toUpperCase()} WALLET · LEVEL 1</small><code>{wallet}</code><p>{isOwn ? 'Click your profile picture to upload and crop an image.' : 'Public player statistics'}</p></div></div>
    <nav className="profile-nav"><button className={section === 'statistics' ? 'active' : ''} onClick={() => setSection('statistics')}>Statistics</button><button className={section === 'transactions' ? 'active' : ''} onClick={() => setSection('transactions')}>Transactions</button>{isOwn && <button className={section === 'settings' ? 'active' : ''} onClick={() => setSection('settings')}>Settings</button>}</nav>
