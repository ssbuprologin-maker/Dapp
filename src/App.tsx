...
import DinoTokensPage from './DinoTokensPage'
...
function App() {
  ...
  const [showDinoTokens, setShowDinoTokens] = useState(false)
  ...
  const keepProfileMenuOpen = () => {
    ...
    setNotificationsOpen(false); setSelectedNotificationId(''); setProfileMenu(true)
  }
  ...
  const openProfile = (section: ProfileSection) => { if (walletAddress) setViewedProfile({ wallet: walletAddress, network: isMegaEth ? 'megaeth' : 'solana' }); setProfileSection(section); setShowProfile(true); setShowAffiliates(false); setShowRewards(false); setShowDinoTokens(false); setProfileMenu(false); setInGame(false) }
  const openProfileButton = () => setProfileMenu(true)
  const openAffiliates = () => { setShowAffiliates(true); setShowProfile(false); setShowRewards(false); setShowDinoTokens(false); setInGame(false); setProfileMenu(false) }
  const openRewards = () => { setShowRewards(true); setShowAffiliates(false); setShowProfile(false); setInGame(false); setProfileMenu(false) }
  const viewChatProfile = (wallet: string, network: 'solana' | 'megaeth') => { setViewedProfile({ wallet, network }); setProfileSection('statistics'); setShowProfile(true); setShowAffiliates(false); setShowRewards(false); setShowDinoTokens(false); setProfileMenu(false); setInGame(false) }
  ...
  useEffect(() => {
    ...
    const refreshWhenVisible = () => { if (document.visibilityState === 'visible') void refreshBalance() }
  ...
  useEffect(() => {
    if (evmAddress || !publicKey) return
    let subscription = 0
    try {
      subscription = connection.onAccountChange(publicKey, account => {
        setBalance(account.lamports / LAMPORTS_PER_SOL)
        setBalanceUnavailable(false)
      }, 'confirmed')
    } catch { /* The five-second refresh remains available if WebSocket subscriptions are unavailable. */ }
    return () => { if (subscription) void connection.removeAccountChangeListener(subscription).catch(() => undefined) }
  }, [connection, evmAddress, publicKey])
  const recordConfirmedSpend = useCallback((amount: number) => {
    setBalance(current => current === null ? current : Math.max(0, current - amount))
    window.setTimeout(() => void refreshBalance(), 750)
  ...
  const deleteChatMessage = async (messageId: string) => {
    ...
    const moderator = isMegaEth ? walletAddress.toLowerCase() : walletAddress
    const timestamp = Date.now()
    const messageToSign = `Testnet Games chat deletion\nModerator: ${network}:${moderator}\nMessage ID: ${messageId}\nTimestamp: ${timestamp}`
    let signature: string
    if (isMegaEth) {
      const provider = getMetaMaskProvider()
      if (!provider) throw new Error('MetaMask is unavailable.')
    ...
    const response = await fetch('/api/chat-history', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ network, moderator, messageId, timestamp, signature }) })
    const responseText = await response.text()
    let body: { message?: string } = {}
    try { body = responseText ? JSON.parse(responseText) as { message?: string } : {} } catch { /* Vercel may return plain text for platform failures. */ }
    if (!response.ok) throw new Error(body.message ?? `Could not delete message (${response.status}).`)
