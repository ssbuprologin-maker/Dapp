...
export default function ChatRail({ wallet, network }: { wallet: string | null; network: Network }) {
  ...
  return <aside className="chat-rail">
    ...
    <div className="chat-feed" ref={feed}>{messages.length ? messages.map(item => <article key={item.id}><header><strong>{item.name}</strong><time>{new Date(item.sentAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</time></header><p>{item.message}</p><small>{item.network === 'solana' ? 'SOL' : 'MEGA'}</small></article>) : <div className="chat-empty">No messages yet.<br />Start the global chat.</div>}</div>
